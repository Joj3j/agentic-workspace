#!/usr/bin/env python3
"""
Deploy epipes + ip-filters across all cached SROS NEs in the 9.168.* subnet.

Usage (after sourcing restconf_env.sh):
  python3 multi_ne_deploy.py [--count N] [--base B] [--filter-base F]
                              [--vlan-start V] [--max-batch M] [--threads T]
                              [--poll-interval P] [--poll-timeout S]
                              [--bulk-window W] [--subnet SUBNET]
"""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Multi-NE deploy (epipe + ip-filter)")
    ...
    p.add_argument("--vlan-start",    type=int,   default=20,       dest="vlan_start",
                   help="Starting VLAN (default: 20)")
    p.add_argument("--max-batch",     type=int,   default=5,        dest="max_batch",
                   help="Max epipes per YANG PATCH (default: 5)")
...
def main() -> None:
    ...
    print(f"Deploy {COUNT} epipes + {COUNT} ip-filters on {len(nes)} NEs")
    print(f"base={BASE}  filter-base={FBASE}  vlan-start={VLAN_START}"
          f"  max-batch={MAX_BATCH}  threads={THREADS}\n")

    # Build all PATCH work items across every NE
    all_work: list[tuple[str, int, int, str, list, str]] = []
    for ne, cfg in sorted(nes.items()):
        batches = s.gen_deploy_batches(COUNT, MAX_BATCH)
        offset = 0
        for k, bsz in enumerate(batches, start=1):
            ids = list(range(BASE + offset, BASE + offset + bsz))
            patch_id = f"smoke-{ne}-b{k}"
            edits: list = []
            for j, sid in enumerate(ids):
                edits.append(s.epipe_edit(j+1, sid, cfg["lag"], VLAN_START + offset + j, cfg["sdp"], COUNT))
            for j in range(bsz):
                edits.append(s.filter_edit(bsz + j + 1, FBASE + offset + j))
    ...
    def _submit(item):
        ...
        with s._print_lock:
            print(f"  {ne:<20} batch {k}/{total_k}  {svc_rng:<40}  req={req_id}  {elapsed:.0f}ms")
    ...
    print(f"\n{ok}/{len(all_work)} patches submitted  submit_wall={t_all_submitted - t_start:.2f}s")
    ...
    print(f"\nPer-NE deploy results — {ok_nes}/{len(ne_req_ids)} NEs fully successful:")
