...
description: >-
  ...
  device-registry test_client, comm-worker-gnmi worker_test_client) against
  K8s-deployed services via SSH tunnel and kubectl port-forward. Use when
  the user wants to run test clients against a K8s cluster, generate test
  data on K8s, or connect to remote gRPC services through a jump host.
---
...
- **Port already in use**: the script auto-kills stale port-forwards via
  ...
  `GNMI_FWD_PORT` in `k8s_test_env.local`.
